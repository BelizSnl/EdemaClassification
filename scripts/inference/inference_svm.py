...
def load_svm_artifacts(model_path: str) -> Dict[str, Any]:
    """Load SVM artifacts (model + preprocessor + metadata)."""
...
def predict_df(df_new: pd.DataFrame, artifacts: Dict[str, Any], topk: int = 3):
    """Run SVM inference for a prepared DataFrame and print top-k results."""
...
def main():
    """CLI entry point for SVM inference."""
